...
export function InputBar({ onSubmit, disabled = false }: Props) {
    ...
    const { showCommandMenu, commandQuery,
        selectedIndex,
        scrollRef,
        handleContentChange,
        resolveCommand,
        ...
        <box width={"100%"} alignItems="center" paddingX={2}>
            <box width={"100%"} border={["left"]} borderColor={"cyan"}>
                <box position="relative" justifyContent="center" paddingX={2} paddingY={1} backgroundColor={"#1A1A24"} width={"100%"} gap={1}>
                    ...
                    <textarea ref={textareaRef} onContentChange={handleTextareaContentChange} keyBindings={TEXTAREA_KEY_BINDINGS} focused={!disabled} placeholder={'Ask anything.."Fix a bug in the code"'} />
                    <StatusBar />
                </box>
            </box>
        </box>
